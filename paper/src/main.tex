...
\begin{enumerate}[nosep, leftmargin=*]
    ...
    \item \textbf{Algebraic duality structure} (Ornelas Brand, 2026d). Proposes 14 fundamental dualities across 6 algebraic layers, validated through domain analysis and neurosymbolic learning. Extends the prime encoding with an ontological structure grounded in classical duality theory. \emph{In preparation.}
\end{enumerate}
...
\begin{itemize}[nosep, leftmargin=*]
    \item \textbf{triadic-engine} --- Core algebraic framework: encoding, validation, subsumption, auditing, and semantic search. Install via \texttt{pip install triadic-engine}. Repository: \url{https://github.com/arturoornelasb/Triadic-Neurosymbolic-Engine}
    \item \textbf{triadic-head} --- Drop-in triadic projection head for any HuggingFace transformer. Install via \texttt{pip install triadic-head}. Repository: \url{https://github.com/arturoornelasb/triadic-microgpt/tree/master/triadic-head}
    \item \textbf{reptimeline} --- Discrete representation lifecycle tracker. Install via \texttt{pip install reptimeline}. Repository: \url{https://github.com/arturoornelasb/reptimeline}
\end{itemize}
