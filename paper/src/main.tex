...
The methods described in this paper are available as open-source Python packages:

\begin{itemize}[nosep, leftmargin=*]
    \item \textbf{triadic-engine} --- The core algebraic framework for encoding, validation, subsumption, auditing, and semantic search. Install via \texttt{pip install triadic-engine}. Repository: \url{https://github.com/arturoornelasb/Triadic-Neurosymbolic-Engine}
    \item \textbf{reptimeline} --- A companion tool for tracking how discrete representations evolve during training: lifecycle events (birth, death, stabilization), phase transitions, bottom-up ontology discovery, and causal intervention verification. Install via \texttt{pip install reptimeline}. Repository: \url{https://github.com/arturoornelasb/reptimeline}
\end{itemize}

\noindent Both packages are released under the Business Source License~1.1 (BUSL-1.1), free for research and non-commercial use.
