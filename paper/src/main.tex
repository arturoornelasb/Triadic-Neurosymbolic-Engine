...
\author{Arturo Ornelas \\ \textit{Independent Researcher} \\ \texttt{arturoornelasb@github}}
\date{}
...
\textbf{Results:} We evaluated approximately 2 million possible semantic chains across $2{,}000$ concepts between two standard Sentence Transformer models (\texttt{all-MiniLM-L6-v2} vs. \texttt{paraphrase-MiniLM-L3-v2}). The process identified $108{,}694$ ($5.4\%$) differing relational paths---instances where one model maintained a short semantic chain between concepts while the other topologically severed it. This provides a direct, exact technique for model traceability.
...
The useful regime ($k\!=\!6$--$12$) is narrow with no principled selection method, analogous to selecting hash tables in multi-probe LSH. However, the true positive rate of subsumption can be improved by replacing random hyperplanes with Principal Component Analysis (PCA) directions derived from the target corpus or through contrastive learning directed by known hypernym/hyponym pairs.
...
The prime encoding enables zero-overhead integration with legacy symbolic systems. Below we show real queries in PostgreSQL and SWI-Prolog using the exact values obtained in Experiment 6 (seed 42).

\begin{lstlisting}[language=SQL, caption={PostgreSQL integration}, label={lst:sql}]
-- =============================================
...
INSERT INTO semantic_concepts (concept_name, prime_encoding) VALUES
    ('King',    3230),   -- 2x5x17x19
    ('Queen',   1615),   --   5x17x19
    ('Man',     85085),  -- 5x7x11x13x17
    ('Woman',   1105),   -- 5x13x17
    ('Dog',     19019),  -- 7x11x13x19
    ('Cat',     6783),   -- 3x7x17x19
    ('Love',    7735),   -- 5x7x13x17
    ('Hate',    1105);   -- 5x13x17

-- 1. Logical Subsumption
SELECT c1.concept_name AS more_specific, c2.concept_name AS subsumed
FROM semantic_concepts c1 CROSS JOIN semantic_concepts c2
WHERE c1.prime_encoding % c2.prime_encoding = 0 AND c1.concept_name != c2.concept_name;

-- 2. Algebraic Composition (LCM)
WITH compose AS (
    SELECT 'King+Dog' AS new_concept,
    ABS(3230::numeric * 19019 / gcd(3230, 19019)) AS lcm_value
)
SELECT new_concept, lcm_value, lcm_value % 3230 AS sub_king, lcm_value % 19019 AS sub_dog;

-- 3. Abductive Gap Analysis
SELECT 'King' AS A, 'Man' AS B, gcd(3230, 85085) AS shared_gcd,
    3230 / gcd(3230, 85085) AS unique_to_king,
    85085 / gcd(3230, 85085) AS unique_to_man;
\end{lstlisting}

\begin{lstlisting}[language=Prolog, caption={SWI-Prolog integration}, label={lst:prolog}]
% =============================================
...
subsumes(A, B) :- concept(A, PhiA), concept(B, PhiB), PhiA mod PhiB =:= 0.

% Operation 2: Composition (LCM)
compose(A, B, LCM) :- concept(A, PhiA), concept(B, PhiB),
    G is gcd(PhiA, PhiB), LCM is abs(PhiA * PhiB) / G.

% Operation 3: Gap Analysis
gap_analysis(A, B, Shared, UniqueA, UniqueB) :- concept(A, PhiA), concept(B, PhiB),
    Shared is gcd(PhiA, PhiB), UniqueA is PhiA / Shared, UniqueB is PhiB / Shared.
\end{lstlisting}
...
We are transparent: the semantic validity depends on LSH projection quality, which is sensitive to hyperparameters. The prime layer adds \emph{algebraic structure} but not \emph{semantic information} beyond what LSH provides.
