...
with tab3:
    ...
    if len(st.session_state.word_list) < 2:
        ...
        with an1: analogy_a = st.selectbox("A", st.session_state.word_list, key="an_a")
        with an2: analogy_b = st.selectbox("B", st.session_state.word_list, index=1, key="an_b")
        with an3: analogy_c = st.selectbox("C", st.session_state.word_list, index=2, key="an_c")
                    ...
                    for i in range(n):
                        for j in range(i + 1, n):
                            ...
                            if dist_A != dist_B:
                                discrepancies += 1
                                # Severity
                                if diff >= 3:   sev = "🔴 CRITICAL"
                                elif diff == 2: sev = "🟠 HIGH"
                                else:           sev = "🟡 MEDIUM"
